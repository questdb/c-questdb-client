...
mod tests {
    ...
    /// Gated recorder for the failed-build barrier test: it blocks until
    /// the gate opens, so a build that returned without draining has
    /// already dropped the events by the time the callback can record them.
    struct GatedConnectionEventState {
        gate: std::sync::Mutex<bool>,
        opened: std::sync::Condvar,
        kinds: std::sync::Mutex<Vec<u32>>,
    }

    unsafe extern "C" fn record_gated_connection_event(
        user_data: *mut libc::c_void,
        event: *const crate::column_sender::questdb_connection_event,
    ) {
        let state = unsafe { &*(user_data as *const GatedConnectionEventState) };
        let mut open = state.gate.lock().unwrap();
        while !*open {
            open = state.opened.wait(open).unwrap();
        }
        drop(open);
        state.kinds.lock().unwrap().push(unsafe { (*event).kind });
    }

    /// The C-ABI half of `questdb-rs`'s
    /// `failed_build_delivers_events_to_a_dropped_builder`. C owns the
    /// options object explicitly, and the documented shape is
    /// build-then-free — so on a failed build `line_sender_opts_free` is
    /// what drops the last handle to the event source, discarding whatever
    ...
    fn failed_build_delivers_connection_events_before_opts_are_freed() {
        ...
        let state_in_opener = Arc::clone(&state);
        let opener = thread::spawn(move || {
            thread::sleep(Duration::from_millis(500));
            *state_in_opener.gate.lock().unwrap() = true;
            state_in_opener.opened.notify_all();
        });
        ...
        unsafe {
            ...
            let conf = format!(
                "ws::addr=127.0.0.1:{port};auth_timeout=200;\
                 reconnect_max_duration_millis=200;connect_timeout=200;"
            );
            let conf_utf8 = line_sender_utf8 {
                len: conf.len(),
                buf: conf.as_ptr() as *const c_char,
            };
            let opts = line_sender_opts_from_conf(conf_utf8, &mut err);
            assert!(!opts.is_null(), "opts_from_conf failed");
            assert!(err.is_null());
            assert!(line_sender_opts_connection_event_handler(
                opts,
                record_gated_connection_event,
            ...
            // The free that would discard undelivered events, immediately
            // after the failed build — the natural C shape.
            line_sender_opts_free(opts);
            assert!(sender.is_null(), "build must fail against a dead endpoint");
            assert!(!err.is_null());
            free_err(&mut err);
        }
        ...
        assert!(
            kinds.contains(
                &crate::column_sender::questdb_connection_event_all_endpoints_unreachable
            ),
