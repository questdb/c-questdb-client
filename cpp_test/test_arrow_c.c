 ...
 *   3. Opens a `questdb_db` against the mock + borrows a `sf_column_sender`.
 *   4. Calls `sf_column_sender_flush_arrow_batch[_at_column]`.
...
static sf_column_sender* mock_borrow_column_sender(
    qwp_mock_c** out_mock,
    questdb_db** out_db)
{
    *out_mock = NULL;
...
static void mock_return_close(
    qwp_mock_c* mock, questdb_db* db, sf_column_sender* conn)
{
    ...
    const char* table, const char* label)
{
    qwp_mock_c* mock;
    questdb_db* db;
    sf_column_sender* conn = mock_borrow_column_sender(&mock, &db);
