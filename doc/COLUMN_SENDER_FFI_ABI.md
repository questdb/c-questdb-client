...
- A `questdb_db` (the pool) is **thread-safe**. Share it across
  threads. `questdb_db_borrow_column_sender` and `questdb_db_return_conn`
...
```
  ...
  questdb_db_connect ───► │  questdb_db (pool)       │
                          ...
                          │   └─ ...                  │
                          └──────────┬────────────────┘
                                     │ borrow_column_sender / return_conn
                                     ▼
                          ┌──────────────────────────┐
                          │  qwpws_conn (borrowed)   │
                          │   ├─ column_sender_chunk │
 ...
 */
QUESTDB_CLIENT_API
qwpws_conn* questdb_db_borrow_column_sender(
    questdb_db* db,
    line_sender_error** err_out);
...
| `i8`                            | INT              | widen i8→i32 (4B/row, sentinel-safe) |
| `i16`                           | INT              | widen i16→i32 (4B/row, sentinel-safe) |
| `i32`                           | LONG             | widen i32→i64 (8B/row, sentinel-safe) |
