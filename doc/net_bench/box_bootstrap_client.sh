...
# Single-ref shallow fetches — no full-history clones. $3 may be an exact SHA
# (GitHub permits fetching an arbitrary SHA directly) or a branch/tag name,
# which resolves to its remote tip at fetch time. Echoes the resolved commit
# so callers can key rebuild markers on the concrete SHA, not the ref name.
shallow_at() { # $1 = dir, $2 = repo url, $3 = SHA or branch/tag
    if [ ! -d "$1/.git" ]; then
        git init -q "$1"
...
C_CLIENT_SHA=$(shallow_at "$WORK/c-questdb-client" \
    https://github.com/questdb/c-questdb-client.git "$QNB_C_CLIENT_COMMIT")
echo "   resolved -> ${C_CLIENT_SHA}"
...
JAVA_CLIENT_SHA=$(shallow_at "$WORK/java-questdb-client" \
    https://github.com/questdb/java-questdb-client.git "$QNB_JAVA_CLIENT_COMMIT")
echo "   resolved -> ${JAVA_CLIENT_SHA}"
if [ "$(cat "$WORK/java-questdb-client/.qnb-built" 2>/dev/null)" != "$JAVA_CLIENT_SHA" ]; then
