...
pub enum ErrorCode {
    ...
    /// `Buffer::append_arrow` was passed a column whose Arrow / QuestDB
    /// kind cannot be persisted to a QuestDB table (e.g. `ARRAY(LONG, N-D)`
    /// is query-result-only on the egress side and has no QWP wire tag for
    /// ingress). Only emitted on the `arrow` feature.
    ArrowUnsupportedColumnKind,

    /// `Buffer::append_arrow` was passed a `RecordBatch` that failed
    /// client-side structural validation (column count vs schema, name
    /// encoding, ARROW C Data Interface invariants on a freshly imported
    /// array, etc.). Only emitted on the `arrow` feature.
    ArrowIngest,
}
