...
/// Regression test: ILP/TCP must not deliver SIGPIPE to the host process
/// when the server closes the socket mid-flush.
///
/// The ingress write path goes through `socket2::Socket::send`, which on
/// Linux calls `send(fd, buf, flags=0)` — no `MSG_NOSIGNAL`. Writing to a
/// peer that has closed the connection therefore raises SIGPIPE in
/// addition to returning EPIPE. With the default signal disposition the
/// kernel kills the process. Rust binaries don't see this because the
/// runtime sets SIGPIPE to `SIG_IGN` at startup (see
/// `library/std/src/sys/pal/unix/mod.rs`), but the FFI's actual audience
/// (Python via cffi/ctypes, C consumers) inherit the OS default and do
/// crash.
///
/// macOS is masked by socket2's `set_common_flags` setting `SO_NOSIGPIPE`
/// at `Socket::new`, so this test is Linux-only.
///
/// Strategy: re-exec ourselves with an env sentinel. The child resets
/// SIGPIPE to `SIG_DFL`, opens a Sender against the parent's listener,
/// and flushes once the listener has closed the accepted socket with
/// `SO_LINGER=0` (RST on the wire). The parent asserts the child's
/// `WTERMSIG` is not SIGPIPE. While the bug is present, the child dies
/// from SIGPIPE and the test fails (red). Once `MSG_NOSIGNAL` is wired
/// through on Linux, `send(2)` returns EPIPE without raising the signal,
/// the child exits 0, and the test passes (green).
#[cfg(all(feature = "sync-sender-tcp", target_os = "linux"))]
const SIGPIPE_CHILD_PORT_ENV: &str = "QUESTDB_INGRESS_SIGPIPE_TEST_PORT";

#[cfg(all(feature = "sync-sender-tcp", target_os = "linux"))]
const SIGPIPE_CHILD_READY_MARKER: &str = "QUESTDB_INGRESS_SIGPIPE_CHILD_READY";
...
fn ingress_tcp_flush_does_not_kill_process_on_closed_peer() -> TestResult {
    ...
    if let Ok(port_str) = std::env::var(SIGPIPE_CHILD_PORT_ENV) {
        run_sigpipe_child(port_str.parse().expect("child port"));
    }

    // Parent. Bind a listener and arm a thread that accepts the child's
    // connection then closes it with SO_LINGER=0 — the RST forces the
    // child's first send(2) after observation to hit a definitively-dead
    // socket, not just a half-closed FIN that the kernel would buffer.
    ...
    let output = std::process::Command::new(exe)
        .env(SIGPIPE_CHILD_PORT_ENV, port.to_string())
        .args([
            "--exact",
            "tests::sender::ingress_tcp_flush_does_not_kill_process_on_closed_peer",
            "--nocapture",
        ])
    ...
    // Sanity: the child actually entered our re-exec code path. Without
    // this, a typo in the `--exact` filter would cause libtest to run
    // zero tests and exit 0 — a silent false green even when the bug
    // is present. The child writes the marker to stderr before any
    // flush attempt.
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains(SIGPIPE_CHILD_READY_MARKER),
        "child driver did not run (filter mismatch?). stderr was:\n{stderr}"
    );

    assert_ne!(
        output.status.signal(),
        Some(libc::SIGPIPE),
        "ingress flush killed the child with SIGPIPE — FFI consumers \
         (Python, C) without SIGPIPE masked will crash on a closed \
         server connection. Fix: pass `MSG_NOSIGNAL` to send(2) on \
         Linux (via `Socket::send_with_flags`)."
    );
    Ok(())
}

#[cfg(all(feature = "sync-sender-tcp", target_os = "linux"))]
fn run_sigpipe_child(port: u16) -> ! {
    use std::io::Write as _;

    // Restore SIGPIPE to the default OS disposition (terminate the
    // process). This is what FFI consumers see; the Rust runtime
    // installs SIG_IGN at startup which would otherwise hide the bug.
    unsafe {
        libc::signal(libc::SIGPIPE, libc::SIG_DFL);
    }

    // Marker for the parent: prove the child entered this code path
    // before we attempt anything that might kill us.
    let mut stderr = std::io::stderr().lock();
    let _ = writeln!(stderr, "{SIGPIPE_CHILD_READY_MARKER}");
    let _ = stderr.flush();
    drop(stderr);

    let conf = format!("tcp::addr=127.0.0.1:{port};protocol_version=2;");
    let mut sender = crate::ingress::Sender::from_conf(&conf).expect("connect");

    // Let the parent's accept + SO_LINGER=0 close land. Without this
    // pause, the child's first flush can complete into the local send
    // buffer before the RST is observed, masking the bug.
    ...
    // Repeated flushes — the first one may slip through the kernel
    // send buffer if the RST hasn't propagated yet, but once the local
    // stack registers the dead peer the next `send(2)` raises SIGPIPE.
    for _ in 0..50 {
        let _ = sender.flush(&mut buf);
        std::thread::sleep(Duration::from_millis(20));
    }

    // Reached only if SIGPIPE was never delivered → bug fixed → green.
    std::process::exit(0);
}
