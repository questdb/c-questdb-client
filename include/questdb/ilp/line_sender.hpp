...
namespace questdb::ilp
{
    ...
    template <typename ClockT, typename DurationT>
    std::enable_if_t<questdb::ilp::better_than_microsec_precision<DurationT>::value, questdb::ilp::timestamp_nanos>
        chrono_to_timestamp(std::chrono::time_point<ClockT, DurationT> tp) {
        return { tp };
    }

    template <typename ClockT, typename DurationT>
    std::enable_if_t<!questdb::ilp::better_than_microsec_precision<DurationT>::value, questdb::ilp::timestamp_micros>
        chrono_to_timestamp(std::chrono::time_point<ClockT, DurationT> tp) {
        return { tp };
    }
