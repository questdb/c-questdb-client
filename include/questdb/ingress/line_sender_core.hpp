...
public:
    ...
    error_kind kind() const noexcept { return _kind; }
    const std::string& idp_error() const noexcept { return _idp_error; }
    const std::string& idp_error_description() const noexcept
    {
        return _idp_error_description;
    }
    std::optional<uint16_t> status() const noexcept { return _status; }
...
[[noreturn]] inline void error::throw_from_c(::questdb_error* c_err)
{
    ...
    throw ::questdb::oidc::error{
        ...
        view.has_status
            ? std::optional<uint16_t>{view.status}
            : std::optional<uint16_t>{},
        view.has_retry_after
            ? std::optional<uint64_t>{view.retry_after_seconds}
...
private:
    ...
    {
        if (::questdb::error::has_oidc_detail(c_err))
