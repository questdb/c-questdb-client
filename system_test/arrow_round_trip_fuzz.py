...
from arrow_ffi import DTS_COLUMN
    ...
    *, null_mode: str, ts_base_us: int,
    ...
    for col_name, spec in kinds:
        if null_mode == "valid":
            ...
            mask = afc.partial_null_mask(rnd, n, null_p=0.3); edge = False
...
class TestArrowRoundTripPerKind(afc.ArrowFuzzBase):
    ...
    def _assert_kind_round_trip(
        ...
        actual_dtype = rb_out.column(0).type
        self.assertEqual(
            str(actual_dtype), str(expected_dtype),
            self.label(f"DataType kind={spec.name}: "
                       f"want {expected_dtype}, got {actual_dtype}"),
        )
        # Metadata round-trips only via the egress-stamped field. Check
        # the keys we know the server / adapter stamps for this kind.
        ...
        for r in range(rb_in.num_rows):
            ev_canon = _canonicalise_value(rb_in.column(0)[r].as_py(), spec)
            av_canon = _canonicalise_value(rb_out.column(0)[r].as_py(), spec)
...
def _canonicalise_value(value, spec: KindSpec):
    ...
    if spec.name == "uuid" and isinstance(value, (bytes, bytearray)):
...
class TestArrowRoundTripFuzz(afc.ArrowFuzzBase):
    ...
    def _run_random_iteration(self, it: int, null_mode: str,
        ...
        pool = _round_trip_capable_kinds()
        ...
        for col_idx, (col_name, spec) in enumerate(kinds):
            for r in range(rb_in.num_rows):
                ev = _canonicalise_value(rb_in.column(col_idx)[r].as_py(), spec)
                av = _canonicalise_value(rb_out.column(col_idx)[r].as_py(), spec)
