...
//! `Chunk`, publishes it, and waits for the server `Ok` watermark, then replies
//! `OK -1` — there is no FSN handle on the column path, and the Python driver
//! defaults a missing value to -1. With `request_durable_ack=on` and an
//! `sf_dir`, the store-and-forward backend retains the frame until it is
...
struct State {
    db: Option<QuestDb>,
    // Accumulated by SEND, drained by FLUSH. Held as owned vectors because a
    // `Chunk` borrows its column slices; building the chunk transiently inside
    // FLUSH keeps the borrow within one scope.
    table: Option<String>,
    v: Vec<i64>,
    ts: Vec<i64>,
}
    ...
    match verb {
        ...
        "FLUSH" => {
                ...
                .ok_or_else(|| "no table".to_string())?;
            {
                ...
                // Publish and wait for the server `Ok` watermark. The frame
                // stays in the sf_dir until durably acked, so a kill-9 of the
                // primary before that durable ack leaves it to replay to the
                // successor — the row sidecar's OK-but-not-durable case.
                sender
