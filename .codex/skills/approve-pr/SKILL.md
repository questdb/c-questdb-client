...
`/tmp/approve-pr-$PR.md` with a file-writing facility. Never interpolate raw
review text into a shell command — reviews contain backticks, quotes, and `$`
characters that break shell quoting. If only shell access is available, encode
the complete body safely before decoding it into the file.
