...
process.env.GITHUB_REPOSITORY = "questdb/c-questdb-client";
process.env.GITHUB_API_URL = "https://api.github.com";
process.env.GITHUB_TOKEN = "stub-token";

const { run, MARKER, CONTEXT, commentBody } = require("./check");

const VALID = "fix(ingress): publish the dispatcher close";
...
function pullRequestEvent(title) {
  ...
  fs.writeFileSync(
    file,
    JSON.stringify({ pull_request: { number: 201, title, head: { sha: "headsha" } } })
  );
...
function mergeGroupEvent() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      merge_group: {
        head_ref: "refs/heads/gh-readonly-queue/main/pr-201-abc123",
        head_sha: "queuesha",
      },
    })
  );
...
async function main() {
  await test("an invalid title fails the status and explains itself once", async () => {
    ...
    assert.equal(status.route, "/repos/questdb/c-questdb-client/statuses/headsha");
  ...
  await test("the status is posted on the head commit, not the merge commit", async () => {
    ...
    const calls = stubApi({});
    await run();
    assert.equal(statusOf(calls).route, "/repos/questdb/c-questdb-client/statuses/headsha");
    assert.ok(
  ...
  await test("fixing the title deletes the comment and turns the status green", async () => {
    ...
    assert.equal(removed[0].route, "/repos/questdb/c-questdb-client/issues/comments/11");
  ...
  await test("a merge group reads the queued title and reports by status only", async () => {
    ...
    assert.ok(calls.some((call) => call.route === "/repos/questdb/c-questdb-client/pulls/201"));
    assert.equal(commentCalls(calls).length, 0, "a merge group must not comment");
    assert.equal(statusOf(calls).route, "/repos/questdb/c-questdb-client/statuses/queuesha");
    assert.equal(statusOf(calls).body.state, "failure");
    assert.equal(process.exitCode, 1);
  });
